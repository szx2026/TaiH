...
const workspaceRefresh = document.querySelector('[data-workspace-refresh]');
const workspaceRefreshResult = document.querySelector('[data-workspace-refresh-result]');
if (workspaceRefresh) {
    const refreshKey = `nc-erp:last-workspace-view:${workspaceRefresh.dataset.stage}`;
    ...
    if (lastViewed && changedItems) workspaceRefreshResult.textContent = `自上次查看后有 ${changedItems} 项更新`;
    workspaceRefresh.addEventListener('click', () => {
        window.localStorage.setItem(refreshKey, String(Date.now()));
        workspaceRefreshResult.textContent = '正在刷新最新内容…';
