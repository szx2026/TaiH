<x-layouts.app :title="($departmentWorkspace['title'] ?? '产品项目').' · NC ERP'">
    ...
    @endphp
    <div class="mb-7 flex flex-wrap items-end justify-between gap-4"><div><p class="text-sm font-semibold dept-text-{{ $stage }}">{{ $departmentWorkspace['eyebrow'] ?? '产品项目' }}</p><h1 class="mt-1 text-3xl font-bold">{{ $departmentWorkspace['title'] ?? '产品项目池' }}</h1><p class="mt-2 text-sm text-slate-500">{{ $departmentWorkspace['description'] ?? '统一查看产品与协作进度。' }}</p></div><div class="text-right"><button type="button" data-workspace-refresh data-stage="{{ $stage }}" class="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-800 hover:bg-slate-50">刷新最新内容</button><p data-workspace-refresh-result class="mt-2 text-xs text-slate-500" aria-live="polite" style="min-height: 1.15rem; line-height: 1.15rem;"></p></div></div>
    ...
    @if($departmentWorkspace && $selectedProject)
        ...
        @php
            ...
            $pendingOperationsSpecifications = $selectedProject->decisions->where('decision_type', 'specification')->where('requested_from_stage', 'website_operations')->where('status', 'resolved')->flatMap(function ($decision) { $withdrawn = data_get($decision->details, 'withdrawn_requested_specifications', []); return collect(data_get($decision->details, 'requested_specifications', []))->reject(fn ($spec) => in_array($spec, $withdrawn, true)); })->map(fn ($specification) => trim((string) $specification))->filter()->unique()->reject(fn ($specification) => $selectedProject->skus->contains(fn ($sku) => $sku->variant_name === $specification))->values();
        @endphp
            @php
                $projectStages = [
            ...
            <section class="mt-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <section class="project-progress-rail" data-project-stage-progress aria-label="项目进程">
                        ...
                        <div class="project-progress-steps" role="list">
                            @foreach($projectStages as $stageCode => $stageMeta)
                                ...
                                <button type="button" role="listitem" data-project-stage-item data-stage-label="{{ $stageMeta['label'] }}" data-stage-description="{{ $stageMeta['description'] }}" aria-pressed="{{ $stageCode === $activeStageCode ? 'true' : 'false' }}" @class(['project-progress-step', 'is-complete' => $stageIndex < $activeStageIndex, 'is-current' => $stageCode === $activeStageCode])>
                                    <span class="project-progress-dot">{{ $stageIndex + 1 }}</span><span>{{ $stageMeta['label'] }}</span>
                                </button>
                            @endforeach
                        </div>
                        <p class="project-progress-detail" data-project-stage-description>当前环节：{{ $labels[$activeStageCode] ?? $activeStageCode }} · {{ data_get($projectStages, $activeStageCode.'.description') }}</p>
                    </section>
                </header>

                @if($stage === 'market_research' && $canEdit)
                    <section class="mt-4 rounded-xl border border-orange-200 bg-orange-50 p-4">
                        <div><p class="text-sm font-semibold text-orange-900">产品资料补充</p><p class="mt-1 text-sm text-orange-800">可修改产品名称；关键词会显示在标题后；详情页与素材参考链接会同步给运营部和创意部查阅。</p></div>
                        <form method="POST" action="{{ route('projects.product-information.update', $selectedProject) }}" class="mt-3 grid gap-3 md:grid-cols-2">@csrf @method('PATCH')<input type="hidden" name="return_url" value="{{ request()->fullUrl() }}"><label class="field-label">产品项目名称 <span>*</span><input name="product_name" value="{{ old('product_name', $selectedProject->product_name) }}" required placeholder="产品项目名称" class="field-input"></label><label class="field-label">产品关键词<input name="keywords" value="{{ old('keywords', $selectedProject->keywords) }}" placeholder="例如：轮胎灯, 自行车配件" class="field-input"><small>多个关键词请用逗号分隔。</small></label><label class="field-label">详情页参考链接<input name="detail_reference_url" type="url" value="{{ old('detail_reference_url', $selectedProject->detail_reference_url) }}" placeholder="粘贴参考详情页链接" class="field-input"><small>供运营部和创意部参考。</small></label><label class="field-label">内容素材参考链接<input name="creative_reference_url" type="url" value="{{ old('creative_reference_url', $selectedProject->creative_reference_url) }}" placeholder="粘贴参考素材/视频链接" class="field-input"><small>供创意部制作素材参考。</small></label><div class="md:col-span-2"><button class="rounded bg-orange-700 px-4 py-2 text-sm font-semibold text-white">保存产品资料</button></div></form>
                    </section>
                @endif
                @if($stage === 'market_research' && $canEdit && $selectedProject->skus->isNotEmpty())
                    <section class="mt-4 rounded-xl border border-orange-200 bg-white p-4">
                        <div class="flex items-center justify-between"><p class="text-sm font-semibold text-orange-900">已录入产品规格</p><a href="{{ route('profit-calculator.index', ['project' => $selectedProject->id]) }}" class="inline-flex items-center gap-1 rounded-md border border-teal-300 bg-teal-50 px-2.5 py-1 text-xs font-semibold text-teal-800 hover:bg-teal-100 transition">一键利润测算 &rarr;</a></div><p class="mt-1 text-sm text-slate-600">可修改产品规格、内部 SKU、采购价和重量；删除规格会同步移除共享资料与 Shopify 页面关联。</p>
                        <div class="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                            @foreach($selectedProject->skus as $sku)
                                <article class="rounded-lg border border-slate-200 p-3 text-sm"><p class="font-semibold text-slate-950">产品规格：{{ $sku->variant_name }}</p><p class="mt-1 text-slate-600">内部 SKU：{{ $sku->sku_code ?: '待生成' }}</p><p class="mt-1 text-slate-500">采购价 ¥{{ $sku->purchase_price ?? '待补' }} · 重量 {{ $sku->weight_g ?? '待补' }}g</p><details class="mt-3 border-t border-slate-100 pt-3"><summary class="cursor-pointer font-semibold text-orange-800">修改规格</summary><form method="POST" action="{{ route('projects.skus.update', [$selectedProject, $sku]) }}" class="mt-3 grid gap-2">@csrf @method('PATCH')<label class="text-xs font-medium text-slate-600">产品规格<input name="variant_name" required value="{{ $sku->variant_name }}" placeholder="产品规格名称" class="field-input mt-1"></label><label class="text-xs font-medium text-slate-600">公司内部 SKU<input name="sku_code" required value="{{ $sku->sku_code }}" placeholder="内部 SKU 编码" class="field-input mt-1"></label><label class="text-xs font-medium text-slate-600">采购价（CNY）<input name="purchase_price" type="number" min="0" step="0.01" value="{{ $sku->purchase_price }}" placeholder="采购价 (CNY)" class="field-input mt-1"></label><label class="text-xs font-medium text-slate-600">重量（g）<input name="weight_g" type="number" min="0" value="{{ $sku->weight_g }}" placeholder="重量 (g)" class="field-input mt-1"></label><button class="rounded bg-orange-700 px-3 py-2 font-semibold text-white">保存修改</button></form></details></article>
                            @endforeach
                        </div>
                    </section>
                @endif
            ...
            <div class="mt-5"><div class="w-full rounded-xl border dept-panel-{{ $stage }} p-5">
                @if($stage === 'market_research')
                    <p class="text-sm font-semibold dept-text-market_research">产品部重点工作</p><h3 class="mt-2 text-lg font-bold">选品、产品规格与货源产品信息</h3><div class="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(280px,1.25fr)]"><div>@php $selectionEvidence = $selectedProject->researchSources->first(); @endphp<p class="text-sm font-semibold">当前选品信息</p>@if($canEdit)<form method="POST" action="{{ route('projects.research-sources.store', $selectedProject) }}" class="mt-2 space-y-3 rounded-lg border border-orange-200 bg-white p-4">@csrf<input type="hidden" name="return_url" value="{{ request()->fullUrl() }}"><label class="field-label">来源平台 <span>*</span><select name="platform" required class="field-input"><option value="tiktok" @selected(old('platform', $selectionEvidence?->platform) === 'tiktok')>TikTok</option><option value="facebook_ads" @selected(old('platform', $selectionEvidence?->platform) === 'facebook_ads')>Facebook 广告库</option><option value="amazon" @selected(old('platform', $selectionEvidence?->platform) === 'amazon')>Amazon</option><option value="independent_store" @selected(old('platform', $selectionEvidence?->platform) === 'independent_store')>独立站</option></select></label><label class="field-label">证据链接 <span>*</span><input name="url" required value="{{ old('url', $selectionEvidence?->url) }}" placeholder="粘贴可验证的链接" class="field-input"></label><label class="field-label">选品理由 <span>*</span><textarea name="evidence_note" required placeholder="说明需求、价格、竞争或广告信号" class="field-input">{{ old('evidence_note', $selectionEvidence?->evidence_note) }}</textarea></label><label class="field-label">详情页参考链接<input name="detail_reference_url" type="url" value="{{ old('detail_reference_url', $selectedProject->detail_reference_url) }}" placeholder="粘贴参考详情页链接" class="field-input"><small>用于运营部和创意部查阅详情页参考。</small></label><label class="field-label">内容素材参考链接<input name="creative_reference_url" type="url" value="{{ old('creative_reference_url', $selectedProject->creative_reference_url) }}" placeholder="粘贴参考素材/视频链接" class="field-input"><small>用于创意部制作素材参考。</small></label><button class="rounded bg-orange-700 px-3 py-2 text-sm font-semibold text-white">{{ $selectionEvidence ? '更新选品信息' : '保存选品信息' }}</button></form>@else<div class="mt-2 rounded-lg border border-orange-100 bg-white p-4 text-sm"><p class="font-semibold text-orange-900">{{ $selectionEvidence?->custom_source_name ?: ($sourceLabels[$selectionEvidence?->platform] ?? '暂未填写') }}</p><a href="{{ $selectionEvidence?->url }}" target="_blank" rel="noreferrer" class="mt-1 block break-all font-medium text-blue-700 underline">{{ $selectionEvidence?->url }}</a><p class="mt-2 text-slate-700">{{ $selectionEvidence?->evidence_note }}</p>@if($selectedProject->detail_reference_url)<p class="mt-2 text-xs font-medium text-slate-500">详情页参考链接：<a href="{{ $selectedProject->detail_reference_url }}" target="_blank" rel="noreferrer" class="text-blue-700 underline break-all">{{ $selectedProject->detail_reference_url }}</a></p>@endif @if($selectedProject->creative_reference_url)<p class="mt-2 text-xs font-medium text-slate-500">内容素材参考链接：<a href="{{ $selectedProject->creative_reference_url }}" target="_blank" rel="noreferrer" class="text-blue-700 underline break-all">{{ $selectedProject->creative_reference_url }}</a></p>@endif</div>@endif</div><figure data-image-update-action="{{ route('projects.image.update', $selectedProject) }}" class="overflow-hidden rounded-xl border border-orange-200 bg-white p-3"><figcaption class="mb-2 text-sm font-semibold text-orange-900">产品主图</figcaption>@if($selectedProject->product_image_path)<img src="{{ asset('storage/'.$selectedProject->product_image_path) }}" alt="{{ $selectedProject->product_name }} 产品主图" width="400" height="256" loading="lazy" decoding="async" class="h-64 w-full rounded-lg object-contain" style="aspect-ratio: 16/9; max-height: 16rem;">@else<div class="flex h-64 items-center justify-center rounded-lg bg-orange-50 text-sm text-orange-800" style="aspect-ratio: 16/9; max-height: 16rem;">暂未上传产品主图</div>@endif</figure></div>
                    ...
                    <section data-product-source-editor class="mt-4 rounded-xl border border-orange-300 bg-white p-5">
                        ...
                        <form method="POST" action="{{ route('projects.sources.store', $selectedProject) }}" class="mt-4 space-y-4">
                            ...
                            <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                                ...
                                <label class="field-label md:col-span-2 xl:col-span-3">产品关键词<input name="keywords" value="{{ old('keywords', $selectedProject->keywords) }}" placeholder="例如：轮胎灯, 自行车配件（多个关键词用逗号分隔）" class="field-input"><small>关键词会显示在项目标题后，供所有部门查阅。</small></label>
                            </div>
            ...
            <section class="mt-5 rounded-xl border border-slate-200 bg-white p-5"><div><h3 class="font-semibold">项目共享资料</h3><p class="mt-1 text-sm text-slate-500">四个部门均可查看。链接可直接打开，规格与对应内部 SKU 结合展示。</p></div><div class="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3"><article class="shared-department-card shared-department-card-product rounded-lg p-4 text-sm"><p class="font-semibold">产品部资料</p><div class="mt-3 space-y-2">@forelse($selectedProject->researchSources as $source)@php $evidenceText = $source->evidence_note; $cleanEvidence = $evidenceText; $extractedKeywords = null; if ($evidenceText && preg_match('/^(.*?)(?:\s*关键词[:：]\s*(.*))$/su', $evidenceText, $matches)) { $cleanEvidence = trim($matches[1]); $extractedKeywords = trim($matches[2]); } @endphp<div class="rounded bg-white p-3"><a href="{{ $source->url }}" target="_blank" rel="noreferrer" class="resource-link font-medium">{{ $source->custom_source_name ?: ($sourceLabels[$source->platform] ?? $source->platform) }}</a>@if($cleanEvidence)<p class="mt-1 text-slate-700 leading-relaxed">{{ $cleanEvidence }}</p>@endif<p class="mt-2 text-xs text-slate-400">提交于 {{ $source->created_at?->format('Y-m-d H:i') }}</p></div>@empty<p>暂未录入选品证据。</p>@endforelse</div>@php $displayKeywords = $selectedProject->keywords ?: ($extractedKeywords ?? null); if ($displayKeywords) { $displayKeywords = preg_replace('/^\s*关键词[:：]\s*/u', '', $displayKeywords); } @endphp @if($displayKeywords)<div class="mt-3 rounded bg-white p-3"><p class="font-medium text-slate-900">产品关键词</p><div class="mt-1.5 flex flex-wrap gap-1.5">@foreach(explode(',', str_replace('，', ',', $displayKeywords)) as $kw)@php $cleanKw = trim(preg_replace('/^\s*关键词[:：]\s*/u', '', $kw)); @endphp @if($cleanKw)<span class="inline-flex items-center rounded bg-orange-100 px-2 py-0.5 text-xs font-semibold text-orange-950">{{ $cleanKw }}</span>@endif @endforeach</div></div>@endif @if($selectedProject->detail_reference_url)<div class="mt-3 rounded bg-white p-3"><p class="font-medium">详情页参考链接</p><a href="{{ $selectedProject->detail_reference_url }}" target="_blank" rel="noreferrer" class="resource-link mt-1 block break-all text-xs text-blue-700 underline">{{ $selectedProject->detail_reference_url }}</a></div>@endif @if($selectedProject->creative_reference_url)<div class="mt-3 rounded bg-white p-3"><p class="font-medium">内容素材参考链接</p><a href="{{ $selectedProject->creative_reference_url }}" target="_blank" rel="noreferrer" class="resource-link mt-1 block break-all text-xs text-blue-700 underline">{{ $selectedProject->creative_reference_url }}</a></div>@endif @if($selectedProject->sources->isNotEmpty())<div class="mt-3 rounded bg-white p-3"><p class="font-medium text-slate-900">厂家货源信息</p><div class="mt-2 space-y-3">@foreach($selectedProject->sources as $source)<div class="{{ !$loop->first ? 'border-t border-slate-100 pt-2' : '' }}"><p class="font-semibold text-slate-800 text-xs">{{ $source->supplier_name }} @if($source->product_name && $source->product_name !== $source->supplier_name) · {{ $source->product_name }} @endif</p><a href="{{ $source->supplier_url }}" target="_blank" rel="noreferrer" class="resource-link mt-1 block break-all text-xs text-blue-700 underline">{{ $source->supplier_url }}</a>@if($source->notes)<p class="mt-1 text-xs text-slate-600 leading-relaxed">{{ $source->notes }}</p>@endif</div>@endforeach</div></div>@endif<div class="mt-3 border-t border-slate-200 pt-3"><div class="flex items-center justify-between mb-2"><p class="font-semibold">产品规格与内部 SKU</p><a href="{{ route('profit-calculator.index', ['project' => $selectedProject->id]) }}" class="text-xs font-semibold text-teal-700 hover:underline">测算利润 &rarr;</a></div><div class="space-y-2">@forelse($selectedProject->skus as $sku)<div class="rounded bg-white p-3"><div class="flex flex-wrap items-baseline justify-between gap-2"><p class="font-semibold text-slate-900">{{ $sku->variant_name }}</p><span class="text-xs text-slate-400">提交于 {{ $sku->created_at?->format('Y-m-d H:i') }}</span></div><p class="mt-1 text-xs text-slate-700">内部 SKU：<span class="font-medium text-slate-900">{{ $sku->sku_code ?: '待回填' }}</span></p>@if($sku->purchase_price !== null || $sku->weight_g !== null)<p class="mt-1 text-xs text-slate-500">采购价 {{ $sku->purchase_price !== null ? '¥'.number_format((float)$sku->purchase_price, 2) : '待补' }} · 重量 {{ $sku->weight_g !== null ? $sku->weight_g.'g' : '待补' }}</p>@endif</div>@empty<p>暂未录入产品规格与内部 SKU。</p>@endforelse</div></div></article><article class="shared-department-card shared-department-card-operations rounded-lg p-4 text-sm"><p class="font-semibold">运营部资料</p><div class="mt-3 space-y-3">@forelse($selectedProject->landingPages as $page)<div class="rounded bg-white p-3"><a href="{{ $page->page_url }}" target="_blank" rel="noreferrer" class="resource-link font-medium">{{ $page->title }}</a><p class="mt-1">关联 SKU {{ $page->skus->count() }} 个</p>@if($page->detail_image_path)<img src="{{ asset('storage/'.$page->detail_image_path) }}" alt="{{ $page->title }}详情页预览" width="300" height="176" loading="lazy" decoding="async" class="mt-3 h-44 w-full rounded object-contain" style="aspect-ratio: 16/9; max-height: 11rem;">@endif</div>@empty<p>暂未创建 Shopify 页面。</p>@endforelse</div></article><article class="shared-department-card shared-department-card-creative rounded-lg p-4 text-sm"><p class="font-semibold">创意部资料</p><div class="mt-3 space-y-2">@forelse($selectedProject->creativeAssets as $asset)@php $assetTypeLabels = ['video' => '视频', 'gif' => '动图', 'archive' => '压缩包']; $renderedTypes = collect($asset->asset_types ?? [$asset->asset_type])->map(fn($t) => $assetTypeLabels[$t] ?? $t)->join('、'); @endphp<div class="rounded bg-white p-3"><div class="flex items-start justify-between gap-2"><p class="font-medium text-slate-900">{{ $asset->title }}</p>@if(auth()->user()?->hasRole('administrator') || auth()->user()?->department?->code === 'content_creative')<form method="POST" action="{{ route('projects.creative-assets.destroy', [$selectedProject, $asset]) }}" onsubmit="return confirm('确定要删除素材「{{ $asset->title }}」吗？');" class="shrink-0">@csrf @method('DELETE')<input type="hidden" name="return_url" value="{{ request()->fullUrl() }}"><button type="submit" class="text-xs text-rose-600 hover:text-rose-800 underline">删除</button></form>@endif</div><p class="mt-1 text-slate-600">{{ $renderedTypes }} · {{ $asset->source_type }}</p>@if($asset->copy_text)<p class="mt-1 text-slate-700 leading-relaxed">{{ $asset->copy_text }}</p>@endif @if($asset->hasStoredFile())<a href="{{ route('projects.creative-assets.download', [$selectedProject, $asset]) }}" class="mt-2 inline-block font-semibold underline text-blue-700">下载文件</a>@elseif($asset->external_url)<a href="{{ $asset->external_url }}" target="_blank" rel="noreferrer" class="resource-link mt-2 block font-semibold">打开参考链接</a>@else<span class="mt-2 block text-xs text-slate-400">仅文案</span>@endif</div>@empty<p>暂未上传素材。</p>@endforelse</div></article></div></section>
            ...
            @if($stage === 'traffic_growth' && $canEdit)<section class="mt-5 rounded-xl border border-rose-200 bg-rose-50 p-5"><h3 class="font-semibold text-rose-900">项目管理</h3><p class="mt-1 text-sm text-rose-800">归档不会删除历史资料；归档后可随时恢复，或彻底永久删除。</p>@if($selectedProject->status === 'archived')<div class="mt-3 flex flex-wrap items-center gap-2"><form method="POST" action="{{ route('projects.restore', $selectedProject) }}">@csrf @method('PATCH')<button class="rounded bg-emerald-600 px-4 py-2 text-sm font-semibold text-white">恢复产品项目</button></form><form method="POST" action="{{ route('projects.force-destroy', $selectedProject) }}" onsubmit="return confirm('确定要永久删除“{{ $selectedProject->product_name }}”吗？此操作将彻底删除所有关联数据且无法恢复！');">@csrf @method('DELETE')<input type="hidden" name="redirect_to" value="index"><button class="rounded bg-rose-600 px-4 py-2 text-sm font-semibold text-white">永久删除产品项目</button></form></div>@else<form method="POST" action="{{ route('projects.archive', $selectedProject) }}" class="mt-3">@csrf @method('PATCH')<button class="rounded bg-rose-600 px-4 py-2 text-sm font-semibold text-white">归档（删除）产品项目</button></form>@endif</section>@endif
            ...
            <article class="rounded-xl border border-yellow-300 bg-yellow-50 p-5">
                ...
                <div class="mt-3 space-y-2">
                    @forelse($selectedProject->creativeAssets as $asset)
                        <div class="flex items-center justify-between gap-3 rounded-lg bg-white p-3 text-sm"><span class="min-w-0 truncate font-medium">{{ $asset->title }}</span><div class="flex shrink-0 items-center gap-2">@if($asset->hasStoredFile())<a href="{{ route('projects.creative-assets.download', [$selectedProject, $asset]) }}" class="shrink-0 rounded bg-yellow-500 px-3 py-2 font-semibold text-slate-950">下载</a>@elseif($asset->external_url)<a href="{{ $asset->external_url }}" target="_blank" rel="noreferrer" class="shrink-0 rounded border border-yellow-500 px-3 py-2 font-semibold text-yellow-950">打开链接</a>@else<span class="text-slate-500">仅文案</span>@endif @if(auth()->user()?->hasRole('administrator') || auth()->user()?->department?->code === 'content_creative')<form method="POST" action="{{ route('projects.creative-assets.destroy', [$selectedProject, $asset]) }}" onsubmit="return confirm('确定要删除素材「{{ $asset->title }}」吗？');" class="inline">@csrf @method('DELETE')<input type="hidden" name="return_url" value="{{ request()->fullUrl() }}"><button type="submit" class="p-1 text-xs text-rose-600 hover:text-rose-800 underline">删除</button></form>@endif</div></div>
...
document.querySelectorAll('[data-add-specification-request]').forEach((button) => {
    button.addEventListener('click', () => {
        ...
        const row = document.createElement('div');
        row.className = 'flex gap-2';
        row.innerHTML = '<input name="requested_specifications[]" required placeholder="例如：两件套 / 礼盒装 / 新尺寸版本" class="field-input min-w-0 flex-1 text-slate-900 bg-white" style="color: #0f172a !important; background-color: #ffffff !important;"><button type="button" data-remove-specification-request class="rounded border border-blue-200 px-3 text-sm text-blue-800">删除</button>';
...
const appendSubmittedAt = (element, submittedAt) => {
    ...
    const timeMs = Date.parse(String(submittedAt).replace(/-/g, '/')) || 0;
    timestamp.dataset.submittedAt = String(timeMs);
...
if (sharedMaterialsGrid) {
    ...
    const researchGroup = productMaterials?.querySelector('.space-y-2');
    const skuSection = Array.from(productMaterials?.querySelectorAll('.border-t') || []).find(el => el.querySelector('p')?.textContent.includes('产品规格'));
    const internalSkuSection = Array.from(productMaterials?.querySelectorAll('.border-t') || []).find(el => el.querySelector('p')?.textContent.includes('内部 SKU'));
    appendSubmittedAtByIndex(researchGroup?.querySelectorAll(':scope > div') || [], submittedRecords.research);
    appendSubmittedAtByIndex(skuSection?.querySelectorAll(':scope > p:not(.font-semibold)') || [], submittedRecords.skus);
    appendSubmittedAtByIndex(internalSkuSection?.querySelectorAll(':scope > p:not(.font-semibold)') || [], submittedRecords.skus);
...
const workspaceRefreshes = document.querySelectorAll('[data-workspace-refresh]');
const workspaceRefreshResults = document.querySelectorAll('[data-workspace-refresh-result]');
if (workspaceRefreshes.length) {
    const stage = workspaceRefreshes[0].dataset.stage || 'default';
    const refreshKey = `nc-erp:last-workspace-view:${stage}`;
    const lastViewed = Number(window.localStorage.getItem(refreshKey) || 0);
    const changedItems = Array.from(document.querySelectorAll('[data-submitted-at]')).filter((item) => Number(item.dataset.submittedAt || 0) > lastViewed).length;
    if (lastViewed && changedItems) {
        workspaceRefreshResults.forEach((res) => { res.textContent = `自上次查看后有 ${changedItems} 项更新`; });
    }
    workspaceRefreshes.forEach((btn) => {
        btn.addEventListener('click', () => {
            window.localStorage.setItem(refreshKey, String(Date.now()));
            workspaceRefreshResults.forEach((res) => { res.textContent = '正在刷新最新内容…'; });
            window.location.reload();
        });
    });
    if (!lastViewed) window.localStorage.setItem(refreshKey, String(Date.now()));
}
...
document.addEventListener('submit', function (e) {
    const form = e.target;
    if (form && form.method && form.method.toLowerCase() === 'post' && !form.querySelector('input[name="return_url"]') && !form.action.includes('/logout')) {
        const input = document.createElement('input');
        input.type = 'hidden';
